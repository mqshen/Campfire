...
class Websocket : NSObject, NSStreamDelegate {
    ...
    func createHTTPRequest() {
        ...
        //self.addHeader(urlRequest, key: headerCookieName, val: Session.sharedInstance.cookie!)
    ...
    func initStreamsWithData(data: NSData, port: Int) {
    ...
    func processResponse(response: WSResponse) -> Bool {
        if response.isFin && response.bytesLeft <= 0 {
            if response.code == .Ping {
                ...
                dispatch_async(dispatch_get_main_queue(),{
                    self.workaroundMethod()
                    self.delegate?.websocketDidReceiveMessage(response.buffer!)
                })
